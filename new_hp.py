...
def swc2topo_segs(img,size,alive,out,threshold):
    ...
    # print(topo_segs)
    # print(topo_segs[:,3])
    filter_segs = topo_segs[np.argwhere(topo_segs[:,3] > 4)].reshape(12,5)
    ...
    for i in filter_segs:
        ...
        while(current != i[1]):
            if out_swc.size == 0:
                ...
                # print(out)
                # print(out_swc.shape)
                # print(alive[current].reshape(1,7).shape)
    ...
    # for stroing the filter_segs
    # for i in filter_segs[0]:
